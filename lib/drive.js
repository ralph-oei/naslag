...
import { escapeDriveValue } from './search.js';
...
/**
 * Server-side full-text zoeken in de inhoud van Drive-bestanden. Levert een platte
 * lijst {id,name,mimeType}; mappen en prullenbak uitgesloten. De aanroeper filtert
 * daarna tot de vault (search.filterVaultMarkdown) — Drive kan niet recursief op één
 * submap zoeken.
 */
export async function searchFullText(query, token) {
  const q = `fullText contains '${escapeDriveValue(query)}' and trashed=false and mimeType != '${FOLDER_MIME}'`;
  const p = new URLSearchParams({
    q,
