...
async function runContent(q, my, titles) {
  ...
  if (!token) { closeSearch(); onReauth(); return; }
  try {
    ...
    // Drive-treffers dragen geen pad; haal het canonieke index-item (met .path) terug.
    const byId = new Map(idx.files.map((f) => [f.id, f]));
    const content = filterVaultMarkdown(hits, vault)
